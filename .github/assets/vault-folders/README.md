...
| Protected-file handoff (11 seconds) | [GIF](protected-file-demo.gif) · [MP4](protected-file-demo.mp4) | Open a protected tile, retry an incorrect password, then reveal the filename and existing viewer after verification.   |
...
| [Protected-file password](protected-file-password.jpg)         | Generic password prompt before the file name or canonical URL is disclosed. |
| [Protected-file password on mobile](protected-file-mobile.jpg) | Responsive password handoff.                                                |
...
The protected-file regression uses a filename-bearing fixture (`Quarterly Budget FY26.txt`).
Anonymous full HTML, DOM, response headers, and actual React Server Component
responses contain neither its filename nor its canonical path on the folder page,
password prompt, or incorrect-password response. The correct password opens the
existing viewer. Revoking the folder link, moving the file, or making it private
blocks the opaque route even when the correct password is supplied.

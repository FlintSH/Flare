...
RUN apk add --no-cache libc6-compat python3 make g++ pkgconfig libunwind-dev
WORKDIR /app
...
# Install curl for healthcheck, OpenSSL for Prisma, and libunwind for segfault handler
RUN apk add --no-cache curl openssl libc6-compat libunwind
